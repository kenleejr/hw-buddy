"""
WebSocket message handling for HW Buddy WebSocket Server
"""
...
from core.session import create_session, remove_session, SessionState, update_session_image
        ...
        async for message in websocket:
            try:
                ...
                if data.get("type") == "join_session":
                    ...
                    client_type = data.get("client_type", "web")  # "web" or "mobile"
                    
                    if session_id:
                        session.session_id = session_id
                        if client_type == "mobile":
                            session.is_mobile_client = True
                        else:
                            session.is_web_client = True
                ...
                # Handle image uploads from mobile clients (legacy WebSocket upload)
                if data.get("type") == "image_upload":
                    if session.is_mobile_client:
                        image_url = data.get("image_url")
                        image_gcs_url = data.get("image_gcs_url")
                        
                        if image_url:
                            update_session_image(session.session_id, image_url, image_gcs_url)
                            logger.info(f"Updated session {session.session_id} with new image: {image_url}")
                            
                            # Notify web clients about new image
                            await websocket.send(json.dumps({
                                "type": "image_received",
                                "image_url": image_url,
                ...
                # Handle image upload notifications (HTTP upload completion)
                if data.get("type") == "image_uploaded":
                    if session.is_mobile_client:
                        logger.info(f"Mobile client notified of HTTP image upload for session {session.session_id}")
                        
                        # Notify web clients that image was uploaded via HTTP
                        await websocket.send(json.dumps({
                            "type": "image_uploaded_notification",
                            "message": "Image uploaded successfully via HTTP"
                        }))
                    continue
                
                # Skip Gemini processing if this is a mobile client (they only send images)
                if session.is_mobile_client:
                    continue
                ...
                # Handle different types of input for Gemini
                if "type" in data:
                    if data["type"] == "audio":
    ...
    while True:
        ...
        try:
            ...
            for function_call in tool_call.function_calls:
                ...
                tool_result = await execute_tool(function_call.name, function_call.args, session.session_id, db)
                ...
                # If image was captured, send it to Gemini in the tool response
                if tool_result.get("success") and tool_result.get("image_data"):
                    # Include the image in the function response to Gemini
                    function_responses.append(
                        types.FunctionResponse(
                            ...
                            response={
                                "success": True,
                                "message": tool_result.get("message", "Image captured"),
                                "image": tool_result.get("image_data")  # Base64 image data
                            }
                        )
                    )
                    
                    # Also send the image directly to Gemini
                    await session.genai_session.send(input={
                        "data": tool_result.get("image_data"),
                        "mime_type": "image/jpeg"
                    })
                else:
                    function_responses.append(
...
async def handle_client(websocket: Any) -> None:
    ...
    try:
        ...
        # Wait for session join before creating Gemini session
        join_message = await websocket.recv()
        join_data = json.loads(join_message)
        
        if join_data.get("type") == "join_session":
            session.session_id = join_data.get("session_id", session_id)
            client_type = join_data.get("client_type", "web")
            ...
            logger.info(f"Client joined session {session.session_id} as {client_type}")
            await websocket.send(json.dumps({
                "type": "session_joined",
            ...
            # Only create Gemini session for web clients
            if session.is_web_client:
                ...
                async with await create_gemini_session() as gemini_session:
                    ...
                    try:
                        ...
                        if "code = 1006" in str(e) or "connection closed abnormally" in str(e).lower():
                            logger.info(f"Browser disconnected or refreshed for session {session.session_id}")
                        else:
                            raise
            else:
                # For mobile clients, just handle image uploads
                await handle_mobile_client(websocket, session)
        ...
        await cleanup_session(session, session.session_id)

async def handle_mobile_client(websocket: Any, session: SessionState) -> None:
    """Handle mobile client connections (image upload only)."""
    try:
        async for message in websocket:
            try:
                data = json.loads(message)
                ...
                if data.get("type") == "image_upload":
                    ...
                    if image_url:
                        update_session_image(session.session_id, image_url, image_gcs_url)
                        logger.info(f"Mobile client uploaded image for session {session.session_id}")
        ...
        if "connection closed" not in str(e).lower():
            logger.error(f"Mobile client connection error: {e}")
        raise